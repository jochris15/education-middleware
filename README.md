## Middleware
Apa itu middleware?
Middleware adalah sebuah fungsi yang memiliki akses kepada request dan response dalam sebuah aplikasi. Middleware itu bisa digunakan untuk memanipulasi data sebelum masuk ke handler terakhir (biasanya controller)

## Cara menggunakan middleware?
- App level middleware = `app.use()`
- Route level middleware = `router.use()`
- Per-route/per-endpoint middleware = `router.get('/endpoint', middlewareFunction, Controller.read)`
